        ...
        try:
            ...
            cmodel.save(
                file='../output/MLP' + str(curr_layers).replace(',', '-').replace('(', '-').replace(')', '-').replace(
                    ' ', '') + curr_activation + '-2val-lib.c', name="classifier")
...
for curr_estimators in estimators_list:
    ...
    try:
        ...
        cmodel.save(file='../output/RF-' + str(curr_estimators) + '-2val-lib.c', name="classifier")
    except Exception as e:
        print('\033[91m' + "Model failed!")
        print(e)
        print('\033[0m')  # end color
...
cmodel.save(file='../output/GNB-2val-lib.c', name="classifier")
...
for curr_criterion in criterion_list:
    try:
        ...
        cmodel.save(file='../output/DT-' + str(curr_criterion) + '-2val-lib.c', name="classifier")
    except Exception as e:
        print('\033[91m' + "Model failed!")
        print(e)
        print('\033[0m')  # end color
...
for curr_kernel in kernel_list:
    try:
        ...
        print("")
        file = open('../output/SVM-' + str(curr_kernel) + '-2val-lib.c', "w")
